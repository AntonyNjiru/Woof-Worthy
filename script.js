...
async function fetchPets(searchTerm = '') {
    try {
        ...
        // Log the full list of pets
        console.log("All Pets:", pets);

        if (searchTerm) {
            pets = pets.filter(pet => pet.breed.toLowerCase().includes(searchTerm) || pet.gender.toLowerCase().includes(searchTerm));
            console.log("Filtered Pets:", pets);  // Log the filtered list of pets
        }
...
function attachLikeEventListeners() {
    document.querySelectorAll('.heart-icon').forEach(icon => {
        icon.removeEventListener('click', handleLike); // Remove any previously added listeners to avoid duplication
...
function setupEventListeners() {
    searchButton.addEventListener('click', () => fetchPets(searchInput.value.toLowerCase()));
}
